...
# Time : 03:41 AM
# Update made : Version 2.0.0 Viscek plus the infected flock using SIR implemented. Acceptance condtion works.
# Important Note: No. of species recovered is less without flocking as opposed to that with flocking!!!
# BUG : Sometimes the 'S' value comes out to be negative
import numpy as np
                ...
                if (j != i):
                    if (((((x[i][t - 1] - x[j][t - 1]) ** 2) + ((y[i][t - 1] - y[j][t - 1]) ** 2)) ** 0.5) < 1):
                        ...
                        if (c[i][t] == SIR[1] and c[j][t] == SIR[0]):
                            ...
                            if (rndm.uniform(0, 1) <(1 - np.exp(-r*S/t))):
